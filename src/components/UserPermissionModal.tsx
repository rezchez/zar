...
export default function UserPermissionModal({
  ...
  return (
    <div className="confirm-backdrop">
      <div className="confirm-dialog permission-modal-dialog-redesigned max-w-4xl w-full" role="dialog" aria-modal="true">
        {/* Header Section */}
        <div className="permission-modal-header-redesigned">
          <div className="permission-modal-user-card">
            <div className="user-avatar-circle">
              {(targetUser.name || targetUser.email).charAt(0).toUpperCase()}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-extrabold text-slate-100">{targetUser.name || 'کاربر بدون نام'}</h2>
                <span className={`user-role-badge-pill role-${targetUser.role}`}>
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              className="reset-all-overrides-btn"
        ...
        {/* Modal Footer */}
        <div className="permission-modal-footer-redesigned">
          <div className="footer-security-note">
            <Info size={15} className="text-amber-400 shrink-0" />
            <span>
              فرمول محاسبه دسترسی موثر: <strong className="text-amber-300 font-mono">نقش + Grant - Deny</strong>.
              هرگونه تغییر پس از ذخیره بلافاصله در سیستم اعمال خواهد شد.
            </span>
          </div>
          ...
          <div className="footer-action-buttons">
            ...
            <button
              type="button"
              className="dashboard-primary-button gap-2 shadow-lg shadow-amber-500/20"
              onClick={() => void handleSave()}
              disabled={saving || loading}
            >
              {saving ? <RefreshCw size={16} className="spin" /> : <Sparkles size={16} />}
