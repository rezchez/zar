  ...
  const handleCanvasMouseMove = (e: React.MouseEvent) => {
    if (isPanning) {
      ...
      if (targetElement) {
        ...
        const maxYMm = pageDimensions.heightMm - targetElement.size.heightMm;

        newXMm = Math.max(0, Math.min(newXMm, maxXMm));
        newYMm = Math.max(0, Math.min(newYMm, maxYMm));

        updateActiveTemplate((prev) => ({
          ...prev,
          elements: prev.elements.map((el) =>
            el.id === draggingElementId ? { ...el, position: { xMm: Number(newXMm.toFixed(1)), yMm: Number(newYMm.toFixed(1)) } } : el,
          ),
        }));
      }
    }
