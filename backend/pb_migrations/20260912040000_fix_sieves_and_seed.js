...
/**
 * Migration: Allow decimal pieces_per_carat in gemstone_sieves and seed 37 sieves
 */
migrate((app) => {
  const sievesCol = app.findCollectionByNameOrId("gemstone_sieves");
  if (sievesCol) {
    const piecesField = sievesCol.fields.getByName("pieces_per_carat");
    if (piecesField) {
      piecesField.onlyInt = false;
      app.save(sievesCol);
    }
