...
export interface ResolvedHoliday extends PersianHolidayEntry {
  date: Date;
  /** True for hijri (lunar) entries, which are resolved via the tabular approximation. */
  approximate: boolean;
}
...
function dayBounds(date: Date): [Date, Date] {
  return [new Date(date.getFullYear(), date.getMonth(), date.getDate()), new Date(date.getFullYear(), date.getMonth(), date.getDate(), 23, 59, 59, 999)];
}
...
export function isHoliday(
  ...
  return getHolidaysInRange(from, to, options).length > 0;
}
