...
export class BaleProvider implements MessengerProvider {
  ...
  private getToken(): string {
    return this.customToken || process.env.BALE_BOT_TOKEN || '';
  }
  ...
  async sendDocument(options: SendDocumentOptions) {
    ...
    if (!token) {
      throw new Error('سرویس بله پیکربندی نشده است. لطفاً توکن ربات را وارد کنید.');
    }
    ...
    if (!response.ok || data?.ok === false) {
      ...
      throw new Error(`خطای بله: ${sanitized}`);
    }
  }
  ...
  async testConnection(overrideToken?: string, chatId?: string): Promise<TestConnectionResult> {
    ...
    if (!token) {
      return { success: false, message: 'توکن ربات بله مشخص نشده است.' };
    }

    try {
      const getMeUrl = `https://tapi.bale.ai/bot${token}/getMe`;
      ...
      if (!meRes.ok || meData?.ok === false) {
        const errorDesc = (meData?.description || 'توکن ربات بله نامعتبر است.').replace(new RegExp(token, 'g'), '***');
        return { success: false, message: `اعتبارسنجی ربات بله ناموفق بود: ${errorDesc}` };
      }

      const bot = meData?.result;
      const botName = bot?.first_name || bot?.username || 'ربات بله';

      if (chatId && chatId.trim() !== '') {
        const sendUrl = `https://tapi.bale.ai/bot${token}/sendMessage`;
        ...
        if (!sendRes.ok || sendData?.ok === false) {
          const sanitized = (sendData?.description || 'ارسال پیام به شناسه کاربری بله با خطا مواجه شد.').replace(new RegExp(token, 'g'), '***');
          return {
            success: false,
      ...
      return {
        success: false,
        message: err instanceof Error ? err.message : 'خطای ارتباط با سرور بله.',
      };
    }
  }
}
