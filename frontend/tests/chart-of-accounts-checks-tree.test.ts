...
describe('Zarfolio — Opening Checks to Chart of Accounts Hierarchy & Versioning Tests', () => {
  describe('Version & Changelog Verification', () => {
    it('APP_VERSION and APP_VERSION_FA are set to 0.1.3 BETA', () => {
      expect(APP_VERSION).toBe('0.1.3 BETA');
      expect(APP_VERSION_FA).toBe('۰.۱.۳ بتا');
    });

    it('CHANGELOG_RELEASES contains 0.1.3 BETA as current active release', () => {
      expect(CHANGELOG_RELEASES.length).toBeGreaterThanOrEqual(2);
      const latest = CHANGELOG_RELEASES[0];
      expect(latest.version).toBe('0.1.3 BETA');
      expect(latest.versionFa).toBe('۰.۱.۳ بتا');
