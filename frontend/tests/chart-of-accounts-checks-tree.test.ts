...
describe('Zarfolio — Opening Checks to Chart of Accounts Hierarchy & Versioning Tests', () => {
  describe('Version & Changelog Verification', () => {
    it('APP_VERSION and APP_VERSION_FA are set to 0.0.9 BETA', () => {
      expect(APP_VERSION).toBe('0.0.9 BETA');
      expect(APP_VERSION_FA).toBe('۰.۰.۹ بتا');
    });

    it('CHANGELOG_RELEASES contains 0.0.9 BETA as current active release', () => {
      expect(CHANGELOG_RELEASES.length).toBeGreaterThanOrEqual(2);
      const latest = CHANGELOG_RELEASES[0];
      expect(latest.version).toBe('0.0.9 BETA');
      expect(latest.versionFa).toBe('۰.۰.۹ بتا');
