...
export default function UserManagement({
  ...
  return (
    <div className="user-management-page">
      ...
      <AnimatePresence>
        {blockTarget ? (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-md p-4 dir-rtl">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 10 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 10 }}
              className="relative w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 shadow-2xl text-right space-y-4"
              role="dialog"
              aria-modal="true"
            >
              ...
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 leading-relaxed">
                آیا مطمئن هستید که می‌خواهید کاربر <strong className="text-rose-600 dark:text-rose-400">«{blockTarget.name || blockTarget.email}»</strong> را مسدود کنید؟
              </p>

              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-600 dark:text-slate-400 block">مدت زمان مسدودی</label>
                <select
                  value={blockDuration}
                  onChange={(event) => setBlockDuration(event.target.value)}
                  className="w-full h-10 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 font-bold"
                >
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  type="button"
                  className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-extrabold text-xs transition-colors shadow-md"
        ...
        {roleChange ? (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-md p-4 dir-rtl">
            ...
            >
              ...
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 leading-relaxed">
                آیا مطمئن هستید نقش کاربر <strong className="text-slate-900 dark:text-slate-100">«{roleChange.user.name || roleChange.user.email}»</strong> به <strong className="text-amber-600 dark:text-amber-400">«{roleLabel(roleChange.role)}»</strong> تغییر کند؟
              </p>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  type="button"
                  className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-extrabold text-xs transition-colors shadow-md"
